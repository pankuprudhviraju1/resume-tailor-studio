/**
 * Prompt building and response parsing, shared by the server path
 * (OpenAI / Gemini keys) and the browser path (local Ollama).
 */
import { z } from "zod";
...
export const TailorInput = z.object({
  ...
  resumeFile: z
    .object({ name: z.string(), mimeType: z.string(), base64: z.string().min(10) })
    .optional(),
});
export type TailorInputType = z.infer<typeof TailorInput>;

export const KeywordInput = z.object({
  engine: EngineSchema,
  jobDescription: z.string().min(20, "Please paste a longer job description."),
});
export type KeywordInputType = z.infer<typeof KeywordInput>;
...
export type LatexInputType = z.infer<typeof LatexInput>;
...
export type ScoreInputType = z.infer<typeof ScoreInput>;

export const SYSTEM_PROMPT = `You are an expert technical recruiter and resume writer.
...
export const KEYWORD_SYSTEM = `You list the keywords an ATS and a human recruiter would scan a candidate's resume for, given one job description.
Rules:
...
export const LATEX_SYSTEM = `You are an expert resume writer who produces compile-ready LaTeX resumes.
...
export const SCORE_SYSTEM = `You are a strict technical recruiter scoring one resume against one job description.
...
export function tailorRequest(data: TailorInputType) {
...
export function keywordRequest(data: KeywordInputType) {
  return {
...
export function latexRequest(data: LatexInputType) {
  const revision =
...
export function scoreRequest(data: ScoreInputType) {
  return {
    system: SCORE_SYSTEM,
    input: `JOB DESCRIPTION:\n${data.jobDescription}\n\nTARGET ATS KEYWORDS:\n${data.keywords}\n\nRESUME (LaTeX source):\n${data.latex}\n\nScore it now.`,
    jsonSchema: { name: "resume_score", schema: SCORE_SCHEMA },
  };
}
...
export function parseScore(raw: string) {
  ...
  return {
    ...
    breakdown: {
      keywordCoverage: clamp(parsed.keyword_coverage),
      relevance: clamp(parsed.relevance),
      impact: clamp(parsed.impact),
      atsClarity: clamp(parsed.ats_clarity),
    },
